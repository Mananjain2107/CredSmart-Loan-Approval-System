...
# ─── PAGE CONFIG ────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="CredSmart · Loan Approval System",
    page_icon="💳",
    layout="wide",
...
# ─── MODEL PERFORMANCE ───────────────────────────────────────────────────────
        ...
        with cols[i]:
            ...
            <div class="metric-card" style="border-color: {border_col};">
                ...
                <div style="margin-top: 1rem; display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem;">
                    <div><div style="color:#6b6b80;">Precision</div><div style="font-weight:600;">{res['Precision']}%</div></div>
                    <div><div style="color:#6b6b80;">Recall</div><div style="font-weight:600;">{res['Recall']}%</div></div>
                    <div><div style="color:#6b6b80;">F1</div><div style="font-weight:600;">{res['F1']}%</div></div>
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        model_metrics = results[selected_model_metric]
        ...
        fig.update_layout(
            ...
            height=250,
            xaxis=dict(gridcolor="#26263a"),
            yaxis=dict(gridcolor="#26263a", range=[0, 100]),
            showlegend=False
        )
